...
const OptionChain = ({ data }) => {
  ...
  const handleMonitor = () => {
    ...
    const tracking = allData
      .map(
        (item) => `${item.details.strike_price} ${item.details.contract_type}`
      )
      .join(", ");
  ...
  const handleSelection = (type, row) => {
    if (type === "call") {
      if (calls.includes(row.details.ticker)) {
        ...
        setCallsRow(
          callsRow.filter((item) => item.details.ticker !== row.details.ticker)
        );
      } else {
      ...
      if (puts.includes(row.details.ticker)) {
        ...
        setPutsRow(
          putsRow.filter((item) => item.details.ticker !== row.details.ticker)
        );
      } else {
  ...
  return (
    <div className="overflow-hidden flex flex-col w-[100%]">
      <div
        ...
        className=" cursor-pointer flex justify-end items-center  text-sm mb-5 "
      >
                  ...
                  >
                    <td className="p-3 border-b text-center border-gray-100">
                      ...
                      row?.last_quote?.bid !== undefined
                        ? (
                            (row?.greeks?.vega /
                              ((row.last_quote.bid + row.last_quote.ask) / 2)) *
                            100
                          ).toFixed(2) + "%"
                        : "N/A"}
                    </td>
