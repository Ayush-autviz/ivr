  ...
  fetchOptionsData: async (tickers, symbol) => {
    try {
      ...
      set((state) => {
        ...
        const strikes = [
          ...new Set(validResults.map((item) => item.details.strike_price)),
        ];
        ...
        vegas.forEach((_, index) => {
          Object.entries(timeframes).forEach(([timeframe, dataPoints]) => {
            if (updatedIvData?.length >= dataPoints) {
              ...
              const endVega = parseFloat(
                slice[slice.length - 1]?.vegas?.[index]
              );

              const startTheta = parseFloat(slice[0]?.thetas?.[index]);
              const endTheta = parseFloat(
              ...
              const previousData =
                updatedIvData[updatedIvData.length - 2]?.timeframeData?.[
                  timeframe
                ]?.[index] ?? null;
              ...
              if (
                previousData &&
                timestamp - previousData.time < timeframeSeconds
              ) {
        ...
        periods.forEach((period) => {
          ...
          if (updatedIvData.length >= periodNum) {
            ...
            const sum = slice.reduce(
              (acc, curr) => acc + parseFloat(curr.averageIV),
              0
            );
